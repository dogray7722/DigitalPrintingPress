...
export function buildBudgetTrackerSheet(
  ...
  // Row 1: Title
  ws.getCell("A1").value = `OTHER EXPENSES`;
  ws.mergeCells("A1:E1");
  styleTitleRow(ws.getRow(1), ts);

  // Row 2: Sub-heading
  ws.getCell("A2").value =
    "For additional / incidental expenses (shopping, tips, fees, etc.) not already tracked on the dedicated tabs. Category dropdown in column B feeds the OVERVIEW budget summary.";
  ws.mergeCells("A2:E2");
  ws.getCell("A2").font = {
    name: ts.fontName,
    size: ts.sizes.data,
  ...
  ws.getCell("D3").protection = { hidden: true };
  ...
  (ws as any).dataValidations.add("B8:B57", {
    ...
    allowBlank: true,
    formulae: [
      '"Transportation,Accommodation,Food,Activities,Shopping,Miscellaneous"'
    ],
    showErrorMessage: false
  } as ExcelJS.DataValidation);
